...
function showNotification(message, type = 'info') {
    ...
    notification.innerHTML = `
        <div class="${bgColor} text-white px-6 py-3 rounded-xl shadow-2xl flex items-center space-x-3 transform scale-100 opacity-100 transition-all duration-300">
...
document.getElementById('submitWithdrawal').addEventListener('click', async () => {
    ...
    if (!amount || amount <= 0) {
        showNotification('กรุณากรอกจำนวนเงินที่ถูกต้อง', 'error');
        return;
    }
        ...
        employeesSnapshot.forEach(doc => {
            ...
            tableHTML += `
                <tr class="hover:bg-gray-50 transition-colors" data-id="${emp.id}">
                    ...
                    <td class="px-4 py-3 text-center">
                        <button class="edit-btn text-primary-500 hover:text-primary-700 mr-3 font-medium" data-id="${emp.id}">แก้ไข</button>
